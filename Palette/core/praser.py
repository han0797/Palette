...
def parse(args):
    ...
    num_gpus = len(opt['gpu_ids']) if opt.get('gpu_ids') is not None else 1
    
    train_batch_size = opt['datasets'][opt['phase']]['dataloader']['args'].get('batch_size', 4)
    
    if 'val_args' in opt['datasets'][opt['phase']]['dataloader']:
        opt['datasets'][opt['phase']]['dataloader']['val_args']['batch_size'] = train_batch_size * num_gpus
    ...
    ''' set log directory '''
    base_dir = opt['path']['base_dir']
    
    next_version = 0
    if os.path.exists(base_dir):
        for dir_name in os.listdir(base_dir):
            if dir_name.startswith(opt['name'] + '_v'):
                try:
                    version_part = dir_name.split('_v')[-1]
                    if version_part.isdigit():
                        ver = int(version_part)
                        if ver >= next_version:
                            next_version = ver + 1
                except ValueError:
                    continue
    ...
    experiments_root = os.path.join(base_dir, '{}_v{}'.format(opt['name'], next_version))
    if opt['phase'] != 'test':
    ...
    if opt['phase'] != 'test':
        for name in os.listdir('.'):
            if name in ['config', 'models', 'core', 'slurm', 'data']:
                shutil.copytree(name, os.path.join(opt['path']['code'], name), ignore=shutil.ignore_patterns("*.pyc", "__pycache__", "Swath_EA"))
